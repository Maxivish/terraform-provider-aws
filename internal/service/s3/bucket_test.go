...
func TestAccS3Bucket_Manage_lifecycleRuleAbortIncompleteMultipartUploadDaysOnly(t *testing.T) {
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_s3_bucket.bucket"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccBucketLifecycleRuleAbortIncompleteMultipartUploadDaysOnlyConfig(rName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckBucketExists(resourceName),
...
func testAccBucketLifecycleRuleAbortIncompleteMultipartUploadDaysOnlyConfig(rName string) string {
	return fmt.Sprintf(`
resource "aws_s3_bucket" "bucket" {
  bucket = %[1]q
